...
import informationsExporterFields from 'src/app/informations/list/informations-exporter-fields';
...
export class InformationsListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await InformationsApi.list(
        filter,
        this.orderBy,
        null,
        null,
