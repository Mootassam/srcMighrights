...
import testimonyExporterFields from 'src/app/testimony/list/testimony-exporter-fields';
...
export class TestimonyListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await TestimonyApi.list(
        filter,
        this.orderBy,
        null,
        null,
