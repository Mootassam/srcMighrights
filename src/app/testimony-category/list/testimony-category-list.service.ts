...
import testimonyCategoryExporterFields from 'src/app/testimony-category/list/testimony-category-exporter-fields';
...
export class TestimonyCategoryListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await TestimonyCategoryApi.list(
        filter,
        this.orderBy,
        null,
        null,
      ...
      new Exporter(
        testimonyCategoryExporterFields,
        'testimonyCategory',
