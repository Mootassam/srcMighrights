...
import informationCategoryExporterFields from 'src/app/information-category/list/information-category-exporter-fields';
...
export class InformationCategoryListService {
  rows = [];
  count = 0;
  loading = false;
  filter: any = {};
  ...
  async doExport() {
    try {
      ...
      const response = await InformationCategoryApi.list(
        filter,
        this.orderBy,
        null,
        null,
      ...
      new Exporter(
        informationCategoryExporterFields,
        'informationCategory',
